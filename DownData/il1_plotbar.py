...
def SelectDisk(snap_num):
    '''
    Input Snapshot number, like snap_num = 135 (z=0)
    Select disk galaxies, return haloID of them.
    '''
    #select halo stellar particles > 20000
    stellarlen = il.groupcat.loadSubhalos('/Raid1/Illustris/Illustris-1/', snap_num, 'SubhaloLenType')[:,4]
    ...
    mas_mask = stellarlen[haloID] > 100000
    haloID = haloID[mas_mask]
    return haloID


StellarHalfmassRads = il.groupcat.loadSubhalos('/Raid1/Illustris/Illustris-1/', 135, 'SubhaloHalfmassRadType')[:,4]
SubhaloPositions = il.groupcat.loadSubhalos('/Raid1/Illustris/Illustris-1/', 135, 'SubhaloPos')
Stellarlen = il.groupcat.loadSubhalos('/Raid1/Illustris/Illustris-1/', 135, 'SubhaloLenType')[:,4]
...
for haloID in ids[:5]:
    ...
    if A2.max() > 0.15:
        ...
        plt.scatter(coor[:,0], coor[:,1], marker='.', label = 'Stellar particles: %d'%Stellarlen[haloID])
        plt.savefig('/Raid0/zhouzb/TNG_a2/barfig/snap_135/%d.png'%haloID)

        plt.close()
        print('Halo %d is barred, figer saved.'%haloID)

print('Disk galaxies number: %d'%len(ids))
print('Barred: %d'%barred)
print('Bar fraction: ')
print(barred / len(ids))
