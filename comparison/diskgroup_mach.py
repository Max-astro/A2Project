...
def SubhaloMatch(tng_subID):
    ...
    try:
        if len(il1_HaloToSubdisk[il1_GID]) == 1:
            return il1_HaloToSubdisk[il1_GID]
        il1_subIDs = il1_HaloToSubdisk[il1_GID]
    except KeyError:
        il1_subIDs = il1_GroupSubhalo[il1_GID]

    dis = SubPositionMatch(tng_subID, il1_subIDs)
...
def SubPositionMatch(tng_subID, il1_subIDs):
    ...
    dis = []
    for i in il1_subIDs:
        dis.append(np.linalg.norm(tng_subCM[tng_subID] - il1_subCM[i]))
    return np.array(dis)
...
#Stellar mass
tng_StellarMass = il.func.loadSubhalos('TNG', 99, "SubhaloHalfmassRadType")[:,4]
il1_StellarMass = il.func.loadSubhalos('il1', 135, "SubhaloHalfmassRadType")[:,4]

#Gas Fraction
tng_GasFraction = il.func.loadSubhalos('TNG', 99, "SubhaloHalfmassRadType")[:,0] / il.func.loadSubhalos('TNG', 99, "SubhaloMassInHalfRad")
il1_GasFraction = il.func.loadSubhalos('il1', 135, "SubhaloHalfmassRadType")[:,0] / il.func.loadSubhalos('il1', 135, "SubhaloMassInHalfRad")
