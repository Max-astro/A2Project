...
def gethalo(haloID, Subhalolen):
    ...
    part = 0
    while(end > 0):
        with h5py.File('/Raid1/Illustris/TNG-100/Snapshot/snap_099.%d.hdf5'%part,'r') as H5File:
...
def sethalo(haloID):
    '''
    Set halo's stellar particles into new coordinate
    '''
    coor = il.snapshot.loadSubhalo('/Raid1/Illustris/TNG/', 99, haloID, partType=4, fields='Coordinates')
    # vel = il.snapshot.loadSubhalo('/Raid1/Illustris/TNG/', 99, haloID, partType=4, fields='Velocities')
    center = groups(99, haloID, 'Subhalo','SubhaloPos')
...
def Ang(haloID):
    ...
    half_r = (groups(99, haloID, 'Subhalo','SubhaloHalfmassRadType'))[4]
    halo_position = groups(99, haloID, 'Subhalo', 'SubhaloPos')
...
def SelectDisk(snap_num):
    ...
    with h5py.File('/Raid0/zhouzb/TNGdata/offsets_099.hdf5','r') as offset:
    ...
    mas_mask = (halolen[haloID] >= 20000) & (halolen[haloID] <40000)
...
for haloID in ids:
    ...
    np.save('/Raid0/zhouzb/TNG_a2/small/%d.npy'%haloID,np.vstack((A2_R,r_list[r_sort])))
