...
def groups_135(data):
    ...
    '''
    if data == 'len' :
        Subhalolen = np.zeros(1, dtype = 'i8')

        for n in range(8):
            with h5py.File('/Raid1/Illustris/Illustris-1/Groupcat/groups_135.%d.hdf5'%n,'r') as f:
                Subhalolen = np.concatenate((Subhalolen, np.array(f['Subhalo']['SubhaloLenType'][:,4])))

        for i in range(1,len(Subhalolen)):
            Subhalolen[i] += Subhalolen[i-1]
...
#load all Subhalo's stellar particles
...
for haloID in DiskHalo_ID:
    ...
    angfile = h5py.File('/Raid0/zhouzb/angular_momentum.hdf5','r')
    ...
    for i in range(halo_particlen):
        ...
        halo_starcoors[i] = halo_starcoors[i] - halo_cm
...
#        op.create_dataset('r_list', data=r_list)
    ...
    np.save('/Raid0/zhouzb/halo_40000/%d.npy'%haloID,np.vstack((A2_R,r_list[r_sort])))
